...
from spark_rapids_tools.common.prop_manager import AbstractPropertiesContainer
from spark_rapids_tools.common.utilities import exec_sys_cmd, ToolLogging
...
class ClusterNode:
    ...
    @classmethod
    def create_worker_node(cls):
        return cls(SparkNodeType.WORKER)

    @classmethod
    def create_master_node(cls):
...
class CMDDriverBase:
    ...
    def get_and_set_env_vars(self):
        ...
        if 'region' in self.cloud_ctxt:
            # set the region to the argument
            self.env_vars.update({'region': self.cloud_ctxt.get('region')})
    ...
    def run_sys_cmd(self, cmd, cmd_input: str = None, fail_ok: bool = False) -> str:
...
class LocalPlatform(PlatformBase):
    """
    Represents a dummy implementation for Non cloud platform.
    """
...
class ClusterBase:
    ...
    def run_cmd_driver(self, ssh_cmd: str, cmd_input: str = None) -> str or None:
        ...
        :param cmd_input: optional argument string used as an input to the command line. i.e., writing to a file.
        :return:
        """
        # get the master node
...
def get_platform(platform_id: Enum) -> Type[PlatformBase]:
    platform_hash = {
        CloudPlatform.DATAPROC: ('spark_rapids_tools.cloud_api.dataproc', 'DataprocPlatform'),
        CloudPlatform.EMR: ('spark_rapids_tools.cloud_api.emr', 'EMRPlatform'),
        CloudPlatform.LOCAL: ('spark_rapids_tools.cloud_api.sp_types', 'LocalPlatform')
    }
